...
describe('POST /api/courses/[courseId]/tests/generate', () => {
  ...
  it('allows a teacher to generate from a course linked through ClassroomCourse', async () => {
    (getCurrentUserId as any).mockResolvedValue(userId);
    (prisma.course.findUnique as any).mockResolvedValue({
      id: courseId,
      createdById: 'different-user',
      title: 'Course Title',
      classroomLinks: [{ classroomId: 'classroom-1' }],
      modules: [{ lessons: [{ title: 'Lesson 1', content: 'Some content' }] }],
    });
    ...
    const response = await POST(new NextRequest('http://localhost/api/courses/course-1/tests/generate', {
      method: 'POST',
