...
const context = routeContext({ fileId: "stored-1" });
const base = {
  ...
  originalName: "lesson.pdf", fileType: "PDF", courseCover: null, attachment: null, imageFor: null, bannerFor: null,
};
...
describe("protected file delivery", () => {
  ...
  it("denies a hidden lesson file even when its ID is known", async () => {
    vi.mocked(prisma.storedFile.findUnique).mockResolvedValue({ ...base, attachment: {
  ...
  it("denies a post file to a non-member", async () => {
    vi.mocked(prisma.storedFile.findUnique).mockResolvedValue({ ...base, attachment: { post: { classroomId: "class-1" } } } as never);
  ...
  it("only streams a ticket screenshot to its reporter or an admin", async () => {
    vi.mocked(prisma.storedFile.findUnique).mockResolvedValue({ ...base, attachment: { report: { userId: "owner-1" } } } as never);
    expect((await GET(new Request("http://localhost/file"), context)).status).toBe(404);

    vi.mocked(prisma.storedFile.findUnique).mockResolvedValue({ ...base, attachment: { report: { userId: "viewer-1" } } } as never);
  ...
  it("streams an attached profile image to authenticated users", async () => {
    vi.mocked(prisma.storedFile.findUnique).mockResolvedValue({ ...base, fileType: "IMAGE", detectedMime: "image/png", imageFor: { id: "owner-1" } } as never);
    expect((await GET(new Request("http://localhost/file"), context)).status).toBe(200);
  });
});
