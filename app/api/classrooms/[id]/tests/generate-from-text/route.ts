...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (sourceText.length < 50 || sourceText.length > 20_000) {
      return NextResponse.json({ error: "Source text must be between 50 and 20,000 characters" }, { status: 400 });
    }
    ...
    if (!inputSafety.safe) {
      return NextResponse.json({ error: "The source text is not appropriate for test generation" }, { status: 400 });
    }
...
${sourceText}`,
    ...
    if (!outputSafety.safe) {
      return NextResponse.json({ error: "The generated test was flagged as inappropriate" }, { status: 400 });
    }

    return NextResponse.json({ test: object, courseId: null });
  } catch (error) {
