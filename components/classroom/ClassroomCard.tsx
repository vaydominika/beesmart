...
export function ClassroomCard({ name, description, subject, role, memberCount, onClick }: ClassroomCardProps) {
    ...
    return (
        <button
            type="button"
            onClick={onClick}
            className="group relative flex min-h-[210px] w-full flex-col overflow-hidden rounded-2xl border border-(--classroom-accent) bg-white p-5 text-left transition-colors duration-200 hover:bg-[var(--classroom-surface-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--classroom-focus-border)]"
        >
            {subject && (
