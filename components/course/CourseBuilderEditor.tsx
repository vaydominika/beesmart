...
export default function CourseBuilderEditor({ lesson, courseId, previewMode = false, autoPublish = true, onLessonUpdate, onSavingChange }: CourseBuilderEditorProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="overflow-hidden rounded-xl border border-[var(--course-line)] bg-[var(--app-surface)]" aria-label="Lesson content editor">
      ...
      <Dialog open={reviewTool !== null} onOpenChange={(open) => { if (!open) { requestController.current?.abort(); setReviewTool(null); } }}>
        <DialogContent className="max-h-[92vh] max-w-4xl overflow-y-auto border-[var(--course-line)] bg-[var(--app-surface)]">
          ...
          {reviewTool && reviewTool !== "improve" && <div className="space-y-4">
            ...
            {toolError && <div role="alert" className="rounded-xl border border-[var(--app-danger-border)] bg-[var(--app-danger-soft)] p-3 text-sm text-[var(--app-danger)]">{toolError}<WorkspaceButton type="button" variant="ghost" size="compact" onClick={() => void runStructuredTool()} className="ml-2">Retry</WorkspaceButton></div>}
          ...
          {reviewTool === "improve" && <div className="space-y-4">
            ...
            {toolError && <div role="alert" className="rounded-xl border border-[var(--app-danger-border)] bg-[var(--app-danger-soft)] p-3 text-sm text-[var(--app-danger)]">{toolError}<WorkspaceButton type="button" variant="ghost" size="compact" onClick={() => void runImprove()} className="ml-2">Retry</WorkspaceButton></div>}
