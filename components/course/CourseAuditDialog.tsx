...
export function CourseAuditDialog({ open, onOpenChange, courseId, onSelectLesson }: { open: boolean; onOpenChange: (open: boolean) => void; courseId: string; onSelectLesson: (lessonId: string) => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto border-[var(--course-line)] bg-white">
        ...
        {error && <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800"><p>{error}</p><WorkspaceButton type="button" variant="secondary" onClick={() => void runAudit()} className="mt-3">Retry</WorkspaceButton></div>}
        {audit && !loading && <div className="space-y-5">
          ...
          <section><h3 className="font-semibold">Strengths</h3><ul className="mt-2 space-y-2">{audit.strengths.map((item, index) => <li key={index} className="flex gap-2 text-sm"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" />{item}</li>)}</ul></section>
          {audit.safetyFlags.length > 0 && <section className="rounded-xl border border-amber-200 bg-amber-50 p-4"><h3 className="flex items-center gap-2 font-semibold"><AlertTriangle className="h-4 w-4" />Safety flags</h3><div className="mt-3 space-y-3">{audit.safetyFlags.map((flag, index) => <button key={index} type="button" disabled={!flag.lessonId} onClick={() => followIssue(flag.lessonId)} className="block w-full rounded-lg bg-white p-3 text-left text-sm disabled:cursor-default"><strong>{flag.reason}</strong><span className="mt-1 block text-xs text-[var(--course-text-muted)]">{flag.contentSnippet}</span></button>)}</div></section>}
